...
public class WoodSet {
	...
	public WoodSet(String name, MapColor topColor, MapColor sideColor) {
		...
		this.fenceGate = ModBlocks.register(name + "_fence_gate", new TrapdoorBlock(blockSetType, AbstractBlock.Settings.create().mapColor(topColor).instrument(Instrument.BASS).strength(3.0F).nonOpaque().allowsSpawning(Blocks::never).burnable()));
		this.pressurePlate = ModBlocks.register(name + "_pressure_plate", new PressurePlateBlock(blockSetType, AbstractBlock.Settings.create().mapColor(topColor).solid().instrument(Instrument.BASS).noCollision().strength(0.5F).burnable().pistonBehavior(PistonBehavior.DESTROY)));
		this.button = ModBlocks.register(name + "_button", Blocks.createWoodenButtonBlock(blockSetType));
		this.sign = ModBlocks.register(name + "_sign", new SignBlock(woodType, AbstractBlock.Settings.create().mapColor(topColor).noCollision().strength(1.0F).sounds(BlockSoundGroup.WOOD).burnable()));
		...
		this.hangingSign = ModBlocks.register(name + "_hanging_sign", new HangingSignBlock(woodType, AbstractBlock.Settings.create().mapColor(topColor).noCollision().strength(1.0F).sounds(BlockSoundGroup.WOOD).burnable()));
