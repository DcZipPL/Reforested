...
public class WoodSet {
	...
	public WoodSet(String name, MapColor topColor, MapColor sideColor) {
		...
		this.door = ModBlocks.register(name + "_door", new DoorBlock(AbstractBlock.Settings.create().mapColor(topColor).instrument(Instrument.BASS).strength(3.0F).nonOpaque().burnable().pistonBehavior(PistonBehavior.DESTROY), BlockSetType.OAK)); // TODO: BlockSetType
		this.trapdoor = ModBlocks.register(name + "_trapdoor", new TrapdoorBlock(AbstractBlock.Settings.create().mapColor(this.planks.getDefaultMapColor()).instrument(Instrument.BASS).strength(3.0F).nonOpaque().sounds(BlockSoundGroup.WOOD).burnable().pistonBehavior(PistonBehavior.DESTROY), BlockSetType.OAK)); // TODO: BlockSetType
		...
		this.fenceGate = ModBlocks.register(name + "_fence_gate", new TrapdoorBlock(AbstractBlock.Settings.create().mapColor(topColor).instrument(Instrument.BASS).strength(3.0F).nonOpaque().allowsSpawning(Blocks::never).burnable(), BlockSetType.OAK)); // TODO: BlockSetType
		this.pressurePlate = ModBlocks.register(name + "_pressure_plate", new PressurePlateBlock(PressurePlateBlock.ActivationRule.EVERYTHING, AbstractBlock.Settings.create().mapColor(topColor).solid().instrument(Instrument.BASS).noCollision().strength(0.5F).burnable().pistonBehavior(PistonBehavior.DESTROY), BlockSetType.OAK)); // TODO: BlockSetType
		this.button = ModBlocks.register(name + "_button", Blocks.createWoodenButtonBlock(BlockSetType.OAK)); // TODO: BlockSetType
		this.sign = ModBlocks.register(name + "_sign", new SignBlock(AbstractBlock.Settings.create().mapColor(topColor).noCollision().strength(1.0F).sounds(BlockSoundGroup.WOOD).burnable(), WoodType.OAK)); // TODO: WoodType
		//this.wallSign = ModBlocks.register(name + "_wall_sign", new WallSignBlock(AbstractBlock.Settings.create().mapColor(topColor).noCollision().strength(1.0F).sounds(BlockSoundGroup.WOOD).burnable(), WoodType.OAK)); // TODO: WoodType
		this.hangingSign = ModBlocks.register(name + "_hanging_sign", new HangingSignBlock(AbstractBlock.Settings.create().mapColor(topColor).noCollision().strength(1.0F).sounds(BlockSoundGroup.WOOD).burnable(), WoodType.OAK)); // TODO: WoodType
